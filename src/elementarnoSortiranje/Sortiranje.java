...
public class Sortiranje { /// sva elementarna sortiranja
	public static <T> void grubaSila(T[] arr, Comparator<T> comparator) {// svaka dva elementa menjamo ako su u
																			// inverziji
		int n = arr.length;
		for (int i = 0; i < n - 1; i++) {
			for (int j = i + 1; j < n; j++) {
	...
	public static <T> void umetanje(T[] arr, Comparator<T> comparator) {
		// do i-te pozicije sortiran, ubacuje se i+1. element gde treba u levom delu
		// tako sto se svi
		// veci od njega pomere desno (da bi ubacili u niz na neki indeks element prvo
		// napravimo mesta tako sto
	...
	public static <T> void razmena(T[] arr, Comparator<T> comparator) {// poredimo svaka dva susedna
		// i menjamo ako su u inverziji, zbog toga posle svake iteracije najveci
		// ,,ispliva'' na kraj niza
		int n = arr.length;
		boolean p = true;
	...
	public static <T> void shell(T[] arr, Comparator<T> comparator) {// umetanje na steroidima
		// napravimo k podnizova, svaki u sebi ima ne susedne elemente(i, i + k, ...) i
		// nad svakim tim radimo umetanje
	...
	public static <T> void comb(T[] arr, Comparator<T> comparator) {// slicno kao shell, samo sto
		// ne delimo niz u k podnizova nego samo trustujemo da neki dalek element od
		// svoje pozicije
		// brze priblizimo gde treba da bude tako sto ga pomerimo umesto za 1 mesto, za
		// k mesta
		...
		razmena(arr, comparator); // od momenta kad k postane 1 se svodi na sortiranje razmenom(ali ispisati na
									// ispitu celokupno razmenu i ovde :))
	}
}
